...
void save_output_txt(int *matrix, int height, int width)
{
    ...
    // Write rows (height) then columns (width)
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
...
{
    // Formato esperado: primera linea "<height> <width>\n" seguido por height filas con width enteros cada una.
    char file_name[PATH_MAX];
...
int save_img_in_txt(const char *img_name, int height, int width)
{
    ...
    // Guardar en matriz_test.txt con cabecera: <orig_height> <orig_width> en la primera linea,
    // seguido por la matriz remuestreada a (height x width)
    ...
    // Escribimos primero las dimensiones originales (src_h src_w)
    fprintf(out, "%d %d\n", src_h, src_w);
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            fprintf(out, "%d ", (int)gray_dst[i * width + j]);
        }
        fprintf(out, "\n");
    }
...
int convert_txt_to_jpg(const char *img_name)
{
    ...
    // Now read header: first line contains rows (height) and cols (width)
    int rows = 0, cols = 0;
    ...
    // Allocate integer buffer for reading
